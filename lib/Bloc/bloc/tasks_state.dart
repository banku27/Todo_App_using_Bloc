...
class TasksState extends Equatable {
  final List<Task> pendingTaks;
  final List<Task> completedTasks;
  final List<Task> favoriteTasks;
  final List<Task> removedTasks;
  const TasksState({
    this.pendingTaks = const <Task>[],
  ...
  List<Object> get props =>
      [pendingTaks, removedTasks, completedTasks, favoriteTasks];

  Map<String, dynamic> toMap() {
    return {
      'pendingTaks': pendingTaks.map((x) => x.toMap()).toList(),
  ...
  factory TasksState.fromMap(Map<String, dynamic> map) {
    return TasksState(
      pendingTaks:
          List<Task>.from(map['pendingTaks']?.map((x) => Task.fromMap(x))),
      completedTasks:
